...
///
fileprivate protocol _MapStringDictionaryDecodableMarker {
...
fileprivate class _MapEncoder : Encoder {
    // MARK: Properties
    ...
    public func container<Key>(keyedBy: Key.Type) -> KeyedEncodingContainer<Key> {
        ...
        let container = _MapKeyedEncodingContainer<Key>(referencing: self, codingPath: self.codingPath, wrapping: topContainer)
    ...
    public func unkeyedContainer() -> UnkeyedEncodingContainer {
        ...
        return _MapUnkeyedEncodingContainer(referencing: self, codingPath: self.codingPath, wrapping: topContainer)
    }
...
fileprivate struct _MapEncodingStorage {
    // MARK: Properties
...
fileprivate struct _MapKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    typealias Key = K
    ...
    fileprivate init(referencing encoder: _MapEncoder, codingPath: [CodingKey], wrapping container: NSMutableDictionary) {
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type, forKey key: Key) -> KeyedEncodingContainer<NestedKey> {
        ...
        let container = _MapKeyedEncodingContainer<NestedKey>(referencing: self.encoder, codingPath: self.codingPath, wrapping: dictionary)
        return KeyedEncodingContainer(container)
    }
    ...
    public mutating func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
        ...
        self.codingPath.append(key)
        defer { self.codingPath.removeLast() }
        return _MapUnkeyedEncodingContainer(referencing: self.encoder, codingPath: self.codingPath, wrapping: array)
    }

    public mutating func superEncoder() -> Encoder {
        return _MapReferencingEncoder(referencing: self.encoder, at: _MapKey.super, wrapping: self.container)
    }
...
fileprivate struct _MapUnkeyedEncodingContainer : UnkeyedEncodingContainer {
    // MARK: Properties
    ...
    /// Initializes `self` with the given references.
    fileprivate init(referencing encoder: _MapEncoder, codingPath: [CodingKey], wrapping container: NSMutableArray) {
        self.encoder = encoder
        self.codingPath = codingPath
        self.container = container
    }
    ...
    public mutating func superEncoder() -> Encoder {
        return _MapReferencingEncoder(referencing: self.encoder, at: self.container.count, wrapping: self.container)
    }
}
...
fileprivate class _MapReferencingEncoder : _MapEncoder {
    ...
    fileprivate init(referencing encoder: _MapEncoder, at index: Int, wrapping array: NSMutableArray) {
        self.encoder = encoder
    ...
    fileprivate init(referencing encoder: _MapEncoder, at key: CodingKey, wrapping dictionary: NSMutableDictionary) {
        self.encoder = encoder
...
fileprivate class _MapDecoder : Decoder {
    // MARK: Properties
    ...
    fileprivate init(referencing container: Any, at codingPath: [CodingKey] = [], options: MapDecoder._Options) {
...
fileprivate struct _MapDecodingStorage {
    // MARK: Properties
...
fileprivate struct _MapKeyedDecodingContainer<K : CodingKey> : KeyedDecodingContainerProtocol {
    typealias Key = K
    ...
    fileprivate init(referencing decoder: _MapDecoder, wrapping container: [String : Any]) {
        ...
        case .custom(let converter):
            self.container = Dictionary(container.map {
                key, value in (converter(decoder.codingPath + [_MapKey(stringValue: key, intValue: nil)]).stringValue, value)
            }, uniquingKeysWith: { (first, _) in first })
        }
    ...
    public func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type, forKey key: Key) throws -> KeyedDecodingContainer<NestedKey> {
        ...
        let container = _MapKeyedDecodingContainer<NestedKey>(referencing: self.decoder, wrapping: dictionary)
        return KeyedDecodingContainer(container)
    }
    ...
    private func _superDecoder(forKey key: CodingKey) throws -> Decoder {
        ...
        return _MapDecoder(referencing: value, at: self.decoder.codingPath, options: self.decoder.options)
    }
...
fileprivate struct _MapUnkeyedDecodingContainer : UnkeyedDecodingContainer {
    // MARK: Properties
    ...
    public mutating func decodeNil() throws -> Bool {
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(Any?.self, DecodingError.Context(codingPath: self.decoder.codingPath + [_MapKey(index: self.currentIndex)], debugDescription: "Unkeyed container is at end."))
        }
    ...
    public mutating func decode(_ type: Bool.Type) throws -> Bool {
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath: self.decoder.codingPath + [_MapKey(index: self.currentIndex)], debugDescription: "Unkeyed container is at end."))
        }
        ...
        guard let decoded = try self.decoder.unbox(self.container[self.currentIndex], as: Bool.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath: self.decoder.codingPath + [_MapKey(index: self.currentIndex)], debugDescription: "Expected \(type) but found null instead."))
        }
...
extension _MapDecoder {
    ...
    fileprivate func unbox(_ value: Any, as type: Int.Type) throws -> Int? {
        ...
        guard NSNumber(value: int) == number else {
            throw DecodingError.dataCorrupted(DecodingError.Context(codingPath: self.codingPath, debugDescription: "Parsed Map number <\(number)> does not fit in \(type)."))
        }
...
fileprivate struct _MapKey : CodingKey {
